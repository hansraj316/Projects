...
class TestFileManager(unittest.TestCase):
    ...
    def test_write_and_read_file(self):
        content = {"test": "data"}
        result = self.file_manager.write_file("test.json", content, file_type="json")
        self.assertEqual(result["status"], "success")

        read_result = self.file_manager.read_file("test.json", file_type="json")
...
class TestAPIConnector(unittest.TestCase):
    def setUp(self):
        self.api_connector = APIConnector(
            base_url="http://api.test.com", api_key="test_key"
        )
